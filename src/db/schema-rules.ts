 ...
 * No new operators or state types; just convenience functions over existing primitives.
 */
...
export interface FieldSchema {
  ...
  name?: string;
  /** Source-provided type — from parent _schema.{field} value.type */
  ingestedType?: string;
 ...
 *
 * @param schemaStates - All EoState objects under the `._schema.` prefix
 * @param schemaPrefix - The prefix string (e.g. "scope._schema.") used to
...
export function groupSchemaStates(
  ...
  for (const state of schemaStates) {
    ...
    // Compute relative path: "fldX", "fldX.type", "fldX.constraint.gte", etc.
    ...
    // Get or create the FieldSchema entry
    ...
    if (parts.length === 1) {
      // Direct field entity: scope._schema.{fieldKey}
      // Extract name and ingested type from the existing flat operand
    ...
    // Deeper paths are ignored — future extension point
  }
 ...
 *
 * @returns Map<fieldKey, full type DEF operand>
 */
