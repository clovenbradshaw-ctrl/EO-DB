...
export function Layout({ session, onLogout, localMode }: LayoutProps) {
  ...
  useEffect(() => {
    ...
    async function setupSpaceStore() {
      ...
      // Persist space metadata to root IndexedDB so the app can find the
      // space without needing Matrix for space discovery.
      {
        const spaceEntry = mergedEntriesRef.current.find(e => e.spaceTarget === selectedSpace);
        persistSpaceMeta({
          spaceId: selectedSpace!,
  ...
  //
  // On `hidden`: save legacy SyncManager snapshots (cheap; only fires for
  //   older spaces that don't use PeerSync).
  //
