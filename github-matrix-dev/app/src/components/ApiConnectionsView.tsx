...
function ApiDataView({
  ...
  const handleFileSelected = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Reset the input so the same file can be uploaded twice in a row.
    if (e.target) e.target.value = '';
    if (!file) return;
    setImportResult(null);
    clearError(config.connectionId);
    try {
      const result = await importRecordsFromFile(config.connectionId, file);
      setImportResult({
        rowsTotal: result.rowsTotal,
        inserted: result.outcomes.ins_emitted,
        updated: result.outcomes.def_emitted,
        unchanged: result.outcomes.no_change,
        failed: result.outcomes.failed + result.outcomes.tombstoned,
        unmappedColumns: result.unmappedColumns,
        rowsWithFallbackId: result.rowsWithFallbackId,
      });
    } catch {
      // Error already surfaced on the store's `errors[cid]`; the banner renders it.
    }
  }, [config.connectionId, importRecordsFromFile, clearError]);
    ...
    <div style={s.container}>
      ...
      <div style={s.dataHeader}>
        ...
        <div style={s.dataHeaderRight}>
          ...
          <button
            ...
            title="Upload a CSV or JSON file into this table. Columns are matched against the saved schema by name."
          >
          ...
          <input
            ref={fileInputRef}
            type="file"
            accept=".csv,.tsv,.json,.ndjson,text/csv,application/json"
            style={{ display: 'none' }}
      ...
      {importResult && (
        <div
          style={{
            ...s.errorBanner,
            background: importResult.failed > 0 ? undefined : '#1f3a1f',
            color: importResult.failed > 0 ? undefined : '#bfe7bf',
          }}
          role="status"
        >
          <span>
            Imported {importResult.rowsTotal} row{importResult.rowsTotal === 1 ? '' : 's'}:{' '}
            {importResult.inserted} new, {importResult.updated} updated, {importResult.unchanged} unchanged
            {importResult.failed > 0 ? `, ${importResult.failed} failed` : ''}
            {importResult.unmappedColumns.length > 0 && (
              <> · unmapped columns: {importResult.unmappedColumns.join(', ')}</>
            )}
            {importResult.rowsWithFallbackId > 0 && (
              <> · {importResult.rowsWithFallbackId} row{importResult.rowsWithFallbackId === 1 ? '' : 's'} had no id column (autogenerated)</>
            )}
          </span>
          <button style={s.errorBannerRetry} onClick={() => setImportResult(null)}>
            Dismiss
          </button>
        </div>
      )}
